...
export const DailyChallengeStep = () => {
  const { setDailyChallenge, nextStep, prevStep } = useQuizStore();
  const { title, subtitle, options } = quizData.entry3_puppy.daily_challenge;

  const handleSelect = (option: string) => {
    setDailyChallenge(option);
    setTimeout(() => {
      nextStep();
    }, 200);
  };
